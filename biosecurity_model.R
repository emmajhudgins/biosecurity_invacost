...
sum<-read.csv('./data/national.sums.csv')
...
n_refs_dam<-dam%>%group_by(TenYear, code)%>%summarize_at("Reference_ID", length)
n_refs_mg<-mg%>%group_by(TenYear, code)%>%summarize_at("Reference_ID", length)
...
combined<-combined[,c(1:3,7,8:11)]
...
##matched data
stwist_merge<-merge(combined, stwist_match, c('code', 'Species'))
...
combined4<-combined4%>%summarize_if(is.numeric, c(mean,length))
combined4$damage_cost<-combined4$damage_cost_fn1*combined4$damage_cost_fn2
combined4$Knowledge<-combined4$`Knowledge funding_fn1`*combined4$`Knowledge funding_fn2`
combined4$pre_inv<-combined4$`Pre-invasion management_fn1`*combined4$`Pre-invasion management_fn2`
combined4$post_inv<-combined4$`Post-invasion management_fn1`*combined4$`Post-invasion management_fn2`
...
combined4<-merge(combined4, n_refs_dam)
combined4<-merge(combined4, n_refs_mg)
combined4$cum_species_fn1[which(is.na(combined4$cum_species_fn1))]<-0

combined4<-combined4%>%group_by(code)%>%mutate(preinv_todate=cumsum(pre_inv), postinv_todate=cumsum(post_inv), knowledge_todate=cumsum(Knowledge))
m<-gam(log(combined4$damage_cost_fn1+1)~log(combined4$Knowledge+1)+log(combined4$pre_inv+1)+log(combined4$post_inv+1)+log(combined4$cum_species_fn1+1)+log(combined4$tot_sr_fn1+1)+s(log(combined4$TenYear+1), k=5)+log(combined4$imports+1)+log(combined4$imports_historical+1)+(combined4$GDP.j+1)+(combined4$Pop.j+1)+combined4$mgmt_delay_fn1+log(combined4$damage_ref+1))
sum_m<-summary(m)
write.csv(sum_m$p.table,file="damage_results.csv")

# checking cumulative model
m_1<-gam(log(combined4$damage_cost_fn1+1)~log(combined4$knowledge_todate+1)+log(combined4$preinv_todate+1)+log(combined4$postinv_todate+1)+log(combined4$cum_species_fn1+1)+log(combined4$tot_sr_fn1+1)+s(log(combined4$TenYear+1), k=5)+log(combined4$imports+1)+log(combined4$imports_historical+1)+(combined4$GDP.j+1)+(combined4$Pop.j+1)+combined4$mgmt_delay_fn1+log(combined4$damage_ref+1))
summary(m_1) # fit not as strong


combined4$mgmt_spend<-combined4$pre_inv+combined4$post_inv


m2<-gam(log(combined4$mgmt_spend+1)~log(combined4$pre_inv+1)+log(combined4$Knowledge+1)+log(combined4$damage_cost+1)+log(combined4$cum_species_fn1+1)+log(combined4$imports+1)+log(combined4$imports_historical+1)+(combined4$GDP.j+1)+log(combined4$Pop.j+1)+combined4$mgmt_delay_fn1+log(combined4$tot_sr_fn1+1)+log(combined4$mg_ref+1))
sum_m2<-summary(m2)
